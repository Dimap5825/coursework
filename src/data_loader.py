...
def input_date():
    answer = input('Введите готовую дату в формате:%d.%m.%Y %H:%M:%S\nИли по умолчанию будет выбрана дата:\n'
                   '15.11.2021 04:57:31\nВвод:')
...
def get_operation_with_range(operation_df:pd.DataFrame, date_end:str) ->pd.DataFrame:
    ...
    """
    logger = get_logger()
    first_date = datetime.datetime.strptime(date_end, "%d.%m.%Y %H:%M:%S").strftime("%01.%m.%Y %00:%00:%00")
...
def get_cards_num_and_sum(operation_df:pd.DataFrame):
    ...
    result = operation_df[['Номер карты', "Сумма платежа",'Кэшбэк']].groupby('Номер карты').sum().reset_index()
    logger.info('Создали список номер карты, кэшбэк и все расходы по ней\n'
