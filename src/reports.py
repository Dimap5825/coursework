...
def save_report(*args):
    ...
    1. Без аргументов:
       ...
       def my_func(...):
           ...
       В этом случае результат сохраняется в PATH_FOR_REPORT.

    2. С аргументом (например, указанием пути к файлу):
       @save_report("custom_path.json")
       def my_func(...):
           ...
       В этом случае результат сохраняется в указанный файл.

    Поддерживаемые типы данных:
    - pandas.DataFrame → конвертируется в список словарей
    - str, list, dict → сохраняются как JSON
    Другие типы → логируются как "неизвестный формат данных".

    Пустой результат или ошибки при записи логируются через logger.

    :param args: Функция (без скобок) или путь к файлу для сохранения.
    :return: Функция-обертка, сохраняющая результат и возвращающая его.
    """
    logger = get_logger()
    ...
    if len(args) == 1 and callable(args[0]):
        ...
        def wrapper(*wrapper_args, **wrapper_kwargs):
            """
            Обертка для функции без аргументов декоратора.

            Выполняет функцию, сохраняет результат в JSON-файл PATH_FOR_REPORT
            и логирует действия или ошибки.

            :param wrapper_args: Позиционные аргументы исходной функции.
            :param wrapper_kwargs: Именованные аргументы исходной функции.
            :return: Результат исходной функции.
            """
        ...
        def decorator(func_obj):
            ...
            def wrapper_custom(*wrapper_args, **wrapper_kwargs):
                """
                Обертка для функции с указанием пути к файлу при декорировании.

                Выполняет функцию, сохраняет результат в JSON-файл file_path
                и логирует действия или ошибки.
                ...
                :return: Результат исходной функции.
                """
                result = func_obj(*wrapper_args, **wrapper_kwargs)
                if result is not None:
                    if isinstance(result, pd.DataFrame):
                        payload = result.to_dict(orient="records")
                        payload = json.dumps(payload, ensure_ascii=False, indent=4)
...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> Dict:
    """
    Возвращает траты по заданной категории за последние три месяца в виде JSON-совместимого словаря.
    ...
    transactions : pd.DataFrame
        Датафрейм с транзакциями. Ожидается, что в нем есть столбцы:
    ...
    -----------
    dict
        JSON-совместимый словарь с ключами:
        - "category" : категория
        - "total" : суммарные траты за последние три месяца
    """
    ...
    # Рассчитываем дату 3 месяца назад (примерно 90 дней)
    three_months_ago = current_date - datetime.timedelta(days=90)

    # Фильтруем транзакции по категории и дате
    filtered_transactions = transactions[
        (transactions["category"] == category) &
        (transactions["date"].apply(lambda x: datetime.datetime.strptime(x, "%Y-%m-%d").date()) >= three_months_ago) &
        (transactions["date"].apply(lambda x: datetime.datetime.strptime(x, "%Y-%m-%d").date()) <= current_date)
    ]

    # Суммируем траты
    total_amount = filtered_transactions["amount"].sum()

    # Формируем JSON-совместимый результат
    result = {
        "category": category,
        "total": total_amount
    }
